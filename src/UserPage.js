...
function UserPage() {
    ...
    const [balance, setBalance] = useState(0); // New state for user's balance
    ...
    const connectWallet = async () => {
        try {
            if (window.ethereum) {
                ...
                fetchUserBalance(accounts[0], contractInstance); // Fetch user balance
            } else {
    ...
    // Fetch User Balance
    const fetchUserBalance = useCallback(async (userAddress, contractInstance) => {
        try {
            const eduTokenAddress = await contractInstance.eduToken();
            const eduToken = new ethers.Contract(eduTokenAddress, ['function balanceOf(address) view returns (uint256)'], contractInstance.provider);
            const balanceAmount = await eduToken.balanceOf(userAddress);
            setBalance(ethers.utils.formatUnits(balanceAmount, 18));
        } catch (error) {
    ...
    const claimRewards = async () => {
        if (contract) {
            try {
                setLoading(true);
                setErrorMessage('');
                // Fetch the current rewards for the user
                const rewardAmount = await contract.getContributorRewards(account);
                const rewardInEdu = ethers.utils.formatUnits(rewardAmount, 18);
                if (parseFloat(rewardInEdu) === 0) {
                ...
                fetchUserBalance(account, contract); // Update balance after claiming
            } catch (error) {
        ...
        <div className="min-h-screen bg-gray-50 font-sans">
            ...
            <main className="p-8 space-y-8">
                ...
                <section className="bg-white p-8 rounded-lg shadow-lg">
                    ...
                    <p className="text-2xl font-bold text-center">{reward} EDU</p>
                    <button
                                ...
                                <ul className="space-y-4">
                                    {contributions.map((contribution) => (
                                        <li key={contribution.id} className="border p-4 rounded-lg shadow">
                                            ...
                                            <p><strong>Reward:</strong> {contribution.reward} EDU</p>
